...
- Use these user-facing labels: `CPU`, `Motherboard`, `Memory`, `Storage`, `Graphics card`, `CPU cooler`, `Power supply`, `Case`, `Case fans`, `Display`, `Unit price`, `Total`, `Budget difference`, `Compatibility`, and `Verify locally`.
...
- Do not convert currencies by default. If the user explicitly asks for an approximate USD or other currency conversion, verify a current exchange rate online, retain the original CNY amount, show the exchange-rate date, and label the converted figure as a currency estimate rather than a local retail price.
...
- Cases, CPU coolers, case fans, and power supplies vary strongly by region. Mark each selected row with a short `verify local equivalent/availability` note.
...
- For a complete build, use the columns `Component | Exact model | Unit price | Local equivalent`. Keep exact product model names.
- Leave `Local equivalent` blank for CPU, motherboard, memory, storage, and graphics-card rows. Fill it only for each selected CPU cooler, power supply, case, and case-fan row, unless a standard row has a documented regional restriction.
...
- Never expose words such as `strict`, `complete`, pass/check counts, skipped checks, or script status. Say `No hard incompatibilities were found` when appropriate, then name only concrete items that still require verification.
...
- Use one natural market note that fits the request instead of repeating a fixed disclaimer:
  ...
  - Currency request: `The USD figure uses the exchange rate checked on YYYY-MM-DD; it is a currency estimate, not a local retail quote.`
...
- Each selected cooler, power supply, case, and case-fan row has its own short local-equivalent note.
